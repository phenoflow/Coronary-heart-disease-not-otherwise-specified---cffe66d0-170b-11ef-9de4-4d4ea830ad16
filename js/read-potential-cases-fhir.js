...
(async () => {
  ...
  await fs.writeFile("coronary-heart-disease-not-otherwise-specified-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("coronary-heart-disease-not-otherwise-specified-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
